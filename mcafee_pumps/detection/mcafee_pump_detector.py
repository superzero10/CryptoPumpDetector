...
print(datetime.time(datetime.now()), 'TesserOcr version:', pkg_resources.get_distribution("tesserocr").version)
...
def track_that_mcafee_bastard():
    ...
    print(datetime.time(datetime.now()), 'Listening for incoming tweets')
...
def process_tweet_if_written_by_tracked_user(tweet):
    ...
    if tweet['user']['screen_name'] == TRACKED_USER and 'retweeted_status' not in tweet.keys() and \
        ...
        print(datetime.time(datetime.now()), TRACKED_USER, 'AUTHORED TWEET')
        print(datetime.time(datetime.now()), tweet)
        print(datetime.time(datetime.now()), "")
        if 'extended_entities' in tweet.keys() and 'media' in tweet['extended_entities'].keys():
            ...
            print(datetime.time(datetime.now()), 'There was no image attached.')


def analyse_ocr(url):
    with PyTessBaseAPI() as api:
        print(datetime.time(datetime.now()), url)
        request.urlretrieve(url, 'mcafeecoin.jpg')
        api.SetImageFile('mcafeecoin.jpg')
        recognized_text = api.GetUTF8Text()
        print(datetime.time(datetime.now()), recognized_text)
        print(datetime.time(datetime.now()), api.AllWordConfidences())
